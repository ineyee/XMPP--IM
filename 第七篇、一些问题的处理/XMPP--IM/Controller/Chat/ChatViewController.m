...
- (void)xmppStream:(XMPPStream *)sender didSendMessage:(XMPPMessage *)message {
    ...
    NSLog(@"===========>消息发送成功：%@===%@", message, message.elementID);
...
- (void)xmppStream:(XMPPStream *)sender didFailToSendMessage:(XMPPMessage *)message error:(NSError *)error {
    ...
    [ProjectHUD showMBProgressHUDToView:kWindow withText:@"连接异常，消息发送失败！\n\n请确认网络连接或重启聊天界面!" atPosition:(MBProgressHUDTextPositionMiddle) autohideAfter:5 completionHandlerAfterAutohide:nil];
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message {
    ...
    NSLog(@"===========>消息接收成功：%@===%@", message, message.elementID);
    
//    NSXMLElement *request = [message elementForName:@"request"];
//    if (request) {// 接收到消息回执的请求
//
...
//
//            // 发送消息回执
//            [[ProjectXMPP sharedXMPP].stream sendElement:msg];
//        }
//    }else {// 接收到了消息回执
//
...
- (void)reloadMessageRecordWithAnimation:(BOOL)flag {
    ...
    NSMutableArray *fetchedObjects = [[[ProjectXMPP sharedXMPP] fecthMessageRecordWithFriendJID:self.friendUser.jid] mutableCopy];
    [fetchedObjects enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {

        XMPPMessageArchiving_Message_CoreDataObject *object = obj;
        
//        if ([((XMPPMessageArchiving_Message_CoreDataObject *)obj).message .type isEqualToString:@"error"]) {
//
//            [fetchedObjects removeObject:obj];
//        }
    }];
    // 将聊天记录存进数组
